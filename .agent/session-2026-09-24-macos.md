...
4. **본문**: `feat/watch-events` — A0–A4·A6 + 프로세스 시트/윈도우, 컬럼 라벨 위치→커맨드
5. **미완**: Phase2 A5(PSI·load·MemAvailable) 보류
6. **산출**: PR로 push 예정 → main 머지 후 다음 스프린트
7. **금지 유지**: Outpost·iStat·com.relay.console·Scrcpy·gfxinfo·material 0
8. **메모**: ps ARGS 기준 — 앱=패키지ID, 네이티브=경로 · 아이콘 adb 제한
